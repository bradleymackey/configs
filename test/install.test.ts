...
import { $ } from "bun";
import { mkdirSync, rmSync, existsSync, readlinkSync, writeFileSync, readdirSync, statSync } from "fs";
...
describe("Installation Script", () => {
  ...
  test("should run in dry-run mode without making changes", async () => {
    ...
    // Run in dry-run mode
    const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --dry-run`.nothrow().text();
  ...
  test("should be idempotent (safe to run multiple times)", async () => {
    ...
    // Run twice
    await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --skip-packages`.nothrow().quiet();
    const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --skip-packages`.nothrow().text();
  ...
  test("should backup and replace existing regular files", async () => {
    ...
    try {
      ...
      throw new Error("Expected .bashrc to be a symlink but it's a regular file");
    }
  });
  ...
  test("should handle missing source files gracefully", async () => {
    ...
    const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --skip-packages`.nothrow();
  ...
  test("should provide verbose output with --verbose flag", async () => {
    const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --dry-run --verbose`.nothrow().text();
  ...
  test("should detect macOS platform", async () => {
    if (process.platform === "darwin") {
      const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --dry-run --skip-packages`.nothrow().text();
  ...
  test("should verify all symlinks in install.ts match actual files", async () => {
    ...
    const actualFiles = readdirSync(HOME_PATH)
      .filter(f => f.startsWith(".") && f !== ".DS_Store" && f !== ".config" && !f.includes("zsh"))
      .sort();
...
describe("Installation Script Syntax", () => {
  ...
  test("node.ts should have valid syntax", async () => {
    const nodePath = join(CONFIGS_ROOT, "install", "node.ts");
    if (existsSync(nodePath)) {
      // Just check that the file can be parsed, don't run it
      const content = await Bun.file(nodePath).text();
      expect(content).toContain("pnpm add -g");
    }
  });

  test("rust.ts should have valid syntax", async () => {
    const rustPath = join(CONFIGS_ROOT, "install", "rust.ts");
    if (existsSync(rustPath)) {
      const content = await Bun.file(rustPath).text();
...
describe("Edge Cases and Error Handling", () => {
  ...
  test("should handle special characters in HOME path", async () => {
    ...
    const result = await $`HOME=${specialHome} bun ${INSTALL_SCRIPT} --skip-packages`.nothrow();
  ...
  test("should handle concurrent runs gracefully", async () => {
    ...
    const run1 = $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --skip-packages`.nothrow().quiet();
    const run2 = $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --skip-packages`.nothrow().quiet();
...
describe("CLI Argument Combinations", () => {
  ...
  test("should handle all flags together", async () => {
    const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --dry-run --skip-packages --verbose`.nothrow().text();
  ...
  test("should handle short flags", async () => {
    const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} -d -s -v`.nothrow().text();
  ...
  test("should reject invalid flags", async () => {
    const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --invalid-flag`.nothrow();
...
describe("Verification Mode", () => {
  ...
  test("should report missing symlinks when nothing is installed", async () => {
    const result = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --verify`.nothrow().text();
    ...
    const exitResult = await $`HOME=${TEST_HOME} bun ${INSTALL_SCRIPT} --verify`.nothrow();
